...
def reasoning_mode(value: str | None = None) -> ReasoningMode:
    try:
        return ReasoningMode(cast(str, value or os.getenv("PRIMA_REASONING_MODE", "adaptive")).lower())
    except ValueError:
...
def reasoning_budget(max_hops: int | None = None, max_context_tokens: int | None = None) -> ReasoningBudget:
    resolved_hops = max_hops if max_hops else int(os.getenv("PRIMA_REASONING_MAX_HOPS", "3"))
    resolved_context = max_context_tokens if max_context_tokens else int(os.getenv("PRIMA_REASONING_MAX_CONTEXT_TOKENS", "1600"))
    return ReasoningBudget(
        max_hops=max(1, resolved_hops),
        ...
        max_context_tokens=max(128, resolved_context),
