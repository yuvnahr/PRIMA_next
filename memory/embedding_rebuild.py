...
def main(argv=None):
    ...
    p.add_argument("--path", default="./memory_db")
    ...
    os.environ["PRIMA_EMBEDDING_MISMATCH"] = "readonly" if a.dry_run else "rebuild"
    repo = ChromaMemoryRepository(a.path)
    notes = repo.list()
    meta = current_embedding_metadata()
    report = {
        "status": "dry-run" if a.dry_run else "rebuilt",
        "count": len(notes),
        "backend": meta,
        "workers": a.workers,
