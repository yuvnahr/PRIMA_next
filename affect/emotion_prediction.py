...
class EmotionPrediction:
    ...
    def __post_init__(self) -> None:
        ...
        if set(selected) != set(self.selected_labels) or ("neutral" in selected and len(selected) > 1):
            raise ValueError("Selected labels must be official and neutral cannot coexist with another label.")
    ...
    def from_probabilities(cls, probabilities: Mapping[str, float], thresholds: Mapping[str, float], model_id: str, model_revision: str | None = None, metadata: Mapping[str, object] | None = None, selected_labels: tuple[str, ...] | None = None) -> "EmotionPrediction":
        ...
        selected = selected_labels or tuple(label for label in LABELS if label != "neutral" and ordered[label] >= float(thresholds.get(label, 0.5))) or ("neutral",)
